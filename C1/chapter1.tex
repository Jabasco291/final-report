...
% -----------------------------------------------------------------------------
This chapter introduces the project's motivation and context for the work, the aims and objectives, the main contributions, and the structure of the remaining chapters.
...
Different \gls{dsg} methods use different approaches to how the scene is processed. This means they differ in how the scene is constructed and labelled. This proposes three core research questions: Which achieves the best accuracy, which is the most practical for deployment, and which creates the richest semantic understanding? This dissertation addresses these questions through a side-by-side evaluation of three state-of-the-art methods, resulting in deploying the best-performing method on real-world data.
...
The main aim of this project was to compare how three state-of-the-art methods perform according to different metrics, which would allow a best-performing to be selected to create a \gls{dsg} of the \gls{hccr} laboratory.
The project was broken down into four clear objectives:

\begin{enumerate}[label=\textbf{O\arabic*:}, leftmargin=*]
  \item \textbf{Deploy all three methods on example data}
    \begin{itemize}
  ...
  \item \textbf{Evaluate results and determine which is the best-performing method}
    \begin{itemize}
      \item Determine the most suitable method for the \gls{hccr} lab in terms of metric, semantic and practical performance
    \end{itemize}
...
\begin{itemize}
  \item  \textbf{Integrated \acrshort{rgbd} camera onto existing 3D \acrshort{lidar} robotic configuration}
    \begin{itemize}
  ...
  \item \textbf{Created simulated office dataset using teleoperated robot}
    ...
    \end{itemize}
  \item \textbf{Deployed three state-of-the-art DSG methods (S-Graphs+ \cite{s_graphs_plus}, Hydra \cite{hydra}, Clio \cite{clio}) on the custom rosbag dataset}
    \begin{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Appendix~\ref{app:supp}}: Supplementary Material and Raw Results.
\end{itemize}
