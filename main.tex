...
\maketitle{%
  ...
  Simultaneous Localisation and Mapping (SLAM) is the fundamental robotics 
  problem of estimating both the pose of a mobile robot and a map of its surroundings. 
  This enables a robot to then make and execute tasks such as planning or navigation. 
  Recent advancements addressing the SLAM problem use a graph-based representation of a 
  ...
  This representation, known as a 3D Dynamic Scene Graph (DSG), encompasses actionable 
  ...
  This project focuses on deploying and evaluating 3 state of the art DSG methods in a 
  simulated environment: Hydra \cite{hydra}, Clio \cite{clio} and S-Graphs+ \cite{s_graphs_plus}. The best performing method 
  was then used to process real world data and create a DSG of the Human Centred Computing 
  and Robotics (HCCR) laboratory. Each method uses a different approach to creating the DSG: 
  S-Graphs+ is geometry based, Hydra adopts a Spatial approach while Clio uses a Task-Driven approach.

  The evaluation revealed that S-Graphs+ struggled in cluttered environments, as objects like desks/beds 
  often prevented the method to incorrectly identify wall planes, leading to incorrect classification of rooms. 
  As well as this, S-Graphs+ is a structural method only, meaning no semantic or object level information is considered. 
  Hydra performed well at identifying objects, with a 65\% recall rate, but struggled to 
  segment all rooms in the scene with an 80\% recall rate, especially struggling with smaller rooms like bathrooms. Clio excelled at 
  segmenting and labelling rooms correctly, however struggled at identifying objects as well as practical application.
}
